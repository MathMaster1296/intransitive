...
const CACHE_FILE = 'tools/mined/cache.json';
...
const themeCount = {};
const sigCount = {};
const LIMITS = { corner: 12, box: 14, fork: 20, stop: 40, capture: 26, trap: 18, race: 44, block: 44, only: 24, material: 22 };
...
for (const c of candidates) {
  ...
  // Cheap pre-check with the mined analysis: skip themes that are already full.
  const preTheme = theme(board, turn, c.rows[0].m, c.kind, { line: '' });
  if ((themeCount[preTheme] || 0) >= LIMITS[preTheme]) continue;
  const key = c.board.join('') + '|' + turn;
  let entry = cache[key];
  if (!entry) {
    const rows = analyze(board, turn, total <= 6 ? 11 : 9, total <= 6 ? 600 : 400);
    const cls = classify(board, turn, rows);
    entry = { rows: rows.map((r) => ({ m: r.m, score: r.score, reply: r.reply })), cls };
    if (cls) entry.pv = principalLine(E.applyMove(board, rows[0].m), 1 - turn, 6);
    cache[key] = entry;
